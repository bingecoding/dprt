...
#include "engines/lightcutsgpu.h"


LightCutsGpuRenderThread::LightCutsGpuRenderThread(const unsigned int index,
                                       const unsigned int seedBase,
                                       const float samplingStart,
                                       OpenCLIntersectionDevice *device,
                                    LightCutsGpuRenderEngine *renderEngine)
{
...
LightCutsGpuRenderThread::~LightCutsGpuRenderThread()
{
    if (m_started)
		Stop();
...
void LightCutsGpuRenderThread::Start()
{
	m_started = true;
    ...
    m_renderThread = new boost::thread(boost::bind(LightCutsGpuRenderThread::RenderThreadImpl, this));
    
    // Set renderThread priority
    bool res = SetThreadRRPriority(m_renderThread);
    if (res && !m_reportedPermissionError) {
        cerr << "[LightCutsGpuRenderThread::" << m_threadIndex << "] Failed to set ray intersection thread priority (you probably need root/administrator permission to set thread realtime priority)" << endl;
...
void LightCutsGpuRenderThread::InitRender()
{
    ...
    cerr << "[LightCutsGpuRenderThread::" << m_threadIndex <<
    ...
    //--------------------------------------------------------------------------
    // Allocate ray and hit buffer for light clusters
    //--------------------------------------------------------------------------
    
    cerr << "[LightCutsGpuRenderThread::" << m_threadIndex <<
    "] Ray cluster buffer size: " << (sizeof(Ray) * globalWorkGroupSize * ARITY / 1024) <<
    "Kbytes" << endl;
    m_raysClusterBuff = new cl::Buffer(oclContext,
                                CL_MEM_READ_WRITE,
                                sizeof(Ray) * globalWorkGroupSize * ARITY);
    deviceDesc->AllocMemory(m_raysClusterBuff->getInfo<CL_MEM_SIZE>());
    
    cerr << "[LightCutsGpuRenderThread::" << m_threadIndex <<
    "] RayHit cluster buffer size: " << (sizeof(RayHit) * globalWorkGroupSize * ARITY / 1024) <<
    "Kbytes" << endl;
    m_hitsClusterBuff = new cl::Buffer(oclContext,
                                CL_MEM_READ_WRITE,
                                sizeof(RayHit) * globalWorkGroupSize * ARITY);
    deviceDesc->AllocMemory(m_hitsClusterBuff->getInfo<CL_MEM_SIZE>());
    ...
    " -D PARAM_LIGHT_COUNT=" << m_renderEngine->m_lightTree->m_lightTreeSize <<
	...
	if (m_kernelsParameters != newKernelParameters) {
        ...
        files.push_back("kernels/lightcutsgpu_kernel.cl");
		...
		cerr << "[LightCutsGpuRenderThread::" << m_threadIndex << "] Suggested work group size: " << m_initFBWorkGroupSize << endl;
...
void LightCutsGpuRenderThread::Interrupt() {
	if (m_renderThread)
		m_renderThread->interrupt();
}

void LightCutsGpuRenderThread::Stop() {
	if (m_renderThread) {
		m_renderThread->interrupt();
		m_renderThread->join();
	...
	delete m_hitsBuff;
    deviceDesc->FreeMemory(m_raysClusterBuff->getInfo<CL_MEM_SIZE>());
    delete m_raysClusterBuff;
    deviceDesc->FreeMemory(m_hitsClusterBuff->getInfo<CL_MEM_SIZE>());
    delete m_hitsClusterBuff;
...
void LightCutsGpuRenderThread::EnqueueInitFrameBufferKernel(const bool clearPBO) {
	cl::CommandQueue &oclQueue = m_intersectionDevice->GetOpenCLQueue();
...
void LightCutsGpuRenderThread::RenderThreadImpl(LightCutsGpuRenderThread *renderThread)
{
	cerr << "[LightCutsGpuRenderThread::" << renderThread->m_threadIndex << "] Rendering thread started" << endl;
	...
	try {
		...
		cerr << "[LightCutsGpuRenderThread::" << renderThread->m_threadIndex << "] Rendering thread halted" << endl;
	} catch (boost::thread_interrupted) {
		cerr << "[LightCutsGpuRenderThread::" << renderThread->m_threadIndex << "] Rendering thread halted" << endl;
	} catch (cl::Error err) {
		cerr << "[LightCutsGpuRenderThread::" << renderThread->m_threadIndex << "] Rendering thread ERROR: " << err.what() << "(" << err.err() << ")" << endl;
	}
    cout << "count: " << cnt << endl;
	oclQueue.enqueueReadBuffer(
                               *(renderThread->m_frameBufferBuff),
