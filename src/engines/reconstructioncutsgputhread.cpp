...
ReconstructionCutsGpuRenderThread::ReconstructionCutsGpuRenderThread(const unsigned int index,
                                       const unsigned int seedBase,
                                       const float samplingStart,
                                       OpenCLIntersectionDevice *device,
                            ReconstructionCutsGpuRenderEngine *renderEngine)
{
    ...
    BBox sceneBox = renderEngine->m_scene->m_bbox;
...
ReconstructionCutsGpuRenderThread::~ReconstructionCutsGpuRenderThread()
{
    if (m_started)
		Stop();
...
void ReconstructionCutsGpuRenderThread::Start()
{
	m_started = true;
    ...
    m_renderThread = new boost::thread(boost::bind(ReconstructionCutsGpuRenderThread::RenderThreadImpl, this));
    
    // Set renderThread priority
    bool res = SetThreadRRPriority(m_renderThread);
    if (res && !m_reportedPermissionError) {
        cerr << "[ReconstructionCutsGpuRenderThread::" << m_threadIndex << "] Failed to set ray intersection thread priority (you probably need root/administrator permission to set thread realtime priority)" << endl;
...
void ReconstructionCutsGpuRenderThread::InitRender()
{
    ...
    const int lightTreeCutSize = m_renderEngine->m_lightTreeCutSize;
    
    assert(((globalWorkGroupSize % BLOCKWH) == 0) && // Global work size per kernel iteration must be a multiple of 4
           ((height % BLOCKWH) == 0)); // Height of image must be a multiple of 4
    assert(globalWorkGroupSize >= BLOCKWH*BLOCKWH);
    /*
    delete[] m_frameBuffer;
    ...
    */
    ...
    cerr << "[ReconstructionCutsGpuRenderThread::" << m_threadIndex <<
    ...
    //--------------------------------------------------------------------------
    // Allocate Light Tree buffer
    //--------------------------------------------------------------------------
    
    int lightsCount = m_renderEngine->m_lightTree->m_lightTreeSize;
    LightCluster *lightTree = m_renderEngine->m_lightTree->m_lightTreeFlat;
    
    cerr << "[ReconstructionCutsGpuRenderThread::" << m_threadIndex <<
    "] Light Tree buffer size: " << (sizeof(LightCluster) * lightsCount / 1024) <<
    "Kbytes" << endl;
    m_lightTreeBuff = new cl::Buffer(oclContext,
                                   CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                   sizeof(LightCluster) * lightsCount,
                                   lightTree);
    deviceDesc->AllocMemory(m_lightTreeBuff->getInfo<CL_MEM_SIZE>());

    //--------------------------------------------------------------------------
    // Allocate Heap and index buffer for Light Tree to determine lightcuts
    //--------------------------------------------------------------------------
    
    // Each thread must have its own heap
    cerr << "[ReconstructionCutsGpuRenderThread::" << m_threadIndex <<
    "] Light cuts heap buffer size: " << (sizeof(HeapNode) * lightTreeCutSize * globalWorkGroupSize / 1024) <<
    "Kbytes" << endl;
    m_lightCutsHeapBuff = new cl::Buffer(oclContext, CL_MEM_READ_WRITE,
                                         sizeof(HeapNode) * lightTreeCutSize * globalWorkGroupSize);
    deviceDesc->AllocMemory(m_lightCutsHeapBuff->getInfo<CL_MEM_SIZE>());

    cerr << "[ReconstructionCutsGpuRenderThread::" << m_threadIndex <<
    "] Light cuts heap index buffer size: " << (sizeof(int) * globalWorkGroupSize / 1024) <<
    "Kbytes" << endl;
    m_heapIndexBuff = new cl::Buffer(oclContext, CL_MEM_READ_WRITE,
                                         sizeof(int)  * globalWorkGroupSize);
    deviceDesc->AllocMemory(m_heapIndexBuff->getInfo<CL_MEM_SIZE>());
    
    //--------------------------------------------------------------------------
    // Allocate bool for each 4x4 block to determine if standard light cuts
    // or reconstruction cuts should be used.
    //--------------------------------------------------------------------------
    
    cerr << "[ReconstructionCutsGpuRenderThread::" << m_threadIndex <<
    "] Light cuts use standard buffer size: " << (sizeof(bool) * globalWorkGroupSize / (BLOCKWH*BLOCKWH) / 1024) <<
    "Kbytes" << endl;
    
    m_standardLightCutsBuff = new cl::Buffer(oclContext, CL_MEM_READ_WRITE,
                                     sizeof(bool) * globalWorkGroupSize / (BLOCKWH*BLOCKWH));
    deviceDesc->AllocMemory(m_standardLightCutsBuff->getInfo<CL_MEM_SIZE>());
    
    //--------------------------------------------------------------------------
    // Four threads in the 4x4 block each compute the samples for each cluster
    // on or above the cut.
    //--------------------------------------------------------------------------
    
    size_t sampleCutSize = lightsCount;
    cerr << "[ReconstructionCutsGpuRenderThread::" << m_threadIndex <<
    "] Sample cuts buffer size: " << (sizeof(SampleGPU) * sampleCutSize * globalWorkGroupSize / BLOCKWH / 1024) <<
    "Kbytes" << endl;
    
    
    m_sampleCutsBuff = new cl::Buffer(oclContext, CL_MEM_READ_WRITE,
                                             sizeof(SampleGPU) * sampleCutSize * globalWorkGroupSize / BLOCKWH);
    deviceDesc->AllocMemory(m_sampleCutsBuff->getInfo<CL_MEM_SIZE>());
    
    //cerr << "size of sample GPU " << sizeof(SampleGPU) << " size of path struct: " << sizeof(PathDL)
    //<< " sizeof ray: " << sizeof(Ray) << " sizeof rayhit: " << sizeof(cl_float3) << endl;
    ...
    " -D PARAM_LIGHT_PATHS=" << m_renderEngine->m_lightPaths <<
    " -D PARAM_LIGHT_DEPTH=" << m_renderEngine->m_depth <<
    " -D PARAM_LIGHT_COUNT=" << lightsCount <<
    " -D PARAM_LIGHTTREE_ARITY=" << ARITY <<
    " -D PARAM_LIGHTTREE_SAMPLESIZE=" << SAMPLESIZE <<
    " -D PARAM_MAXIMUM_CUT_SIZE=" << lightTreeCutSize <<
    ...
    " -D PARAM_DEVICE_TYPE=" << deviceDesc->GetType() <<
    " -D PARAM_SEED=" << m_renderEngine->m_seed <<
    " -D PARAM_SAMPLE_CUT_SIZE=" << sampleCutSize <<
    ...
    " -D PARAM_SCENE_DIAG=" << m_sceneDiag
    ;
	...
	if (m_kernelsParameters != newKernelParameters) {
        ...
        files.push_back("kernels/reconstructioncutsgpu_kernel.cl");
		...
		try {
			...
			cerr << "[ReconstructionCutsGpuRenderThread::" << m_threadIndex <<
            "] ReconstructionCutsGpu compilation error:\n" << strError.c_str() << endl;
		...
		cerr << "[ReconstructionCutsGpuRenderThread::" << m_threadIndex << "] Suggested work group size: " << m_initFBWorkGroupSize << endl;
    ...
    // Allocate frame buffer on host using pinned memory.
    cl::Buffer *cmPinnedBufOut = new cl::Buffer(oclContext,
                                           CL_MEM_WRITE_ONLY | CL_MEM_ALLOC_HOST_PTR,
                                           sizeof(PixelGPU) * pixelCount);
    
    m_frameBuffer = (PixelGPU *)oclQueue.enqueueMapBuffer(*cmPinnedBufOut, CL_FALSE,
                                                          CL_MAP_READ, 0, sizeof(PixelGPU) * pixelCount);
    //m_frameBuffer = new PixelGPU[pixelCount];
    ...
    m_integratorKernel->setArg(argIndex++, *m_lightTreeBuff);
    m_integratorKernel->setArg(argIndex++, *m_sampleCutsBuff);
    m_integratorKernel->setArg(argIndex++, *m_lightCutsHeapBuff);
    m_integratorKernel->setArg(argIndex++, *m_heapIndexBuff);
    m_integratorKernel->setArg(argIndex++, *m_standardLightCutsBuff);
    ...
    //m_integratorKernel->setArg(argIndex++,  cl::__local(sizeof(int) * wrkGrp));
    // Textures
    if (m_texMapRGBBuff)
...
void ReconstructionCutsGpuRenderThread::Interrupt() {
	if (m_renderThread)
		m_renderThread->interrupt();
}

void ReconstructionCutsGpuRenderThread::Stop() {
	if (m_renderThread) {
		m_renderThread->interrupt();
		m_renderThread->join();
    ...
    deviceDesc->FreeMemory(m_lightTreeBuff->getInfo<CL_MEM_SIZE>());
    delete m_lightTreeBuff;
    deviceDesc->FreeMemory(m_lightCutsHeapBuff->getInfo<CL_MEM_SIZE>());
    delete m_lightCutsHeapBuff;
    deviceDesc->FreeMemory(m_heapIndexBuff->getInfo<CL_MEM_SIZE>());
    delete m_heapIndexBuff;
    deviceDesc->FreeMemory(m_standardLightCutsBuff->getInfo<CL_MEM_SIZE>());
    delete m_standardLightCutsBuff;
    deviceDesc->FreeMemory(m_sampleCutsBuff->getInfo<CL_MEM_SIZE>());
    delete m_sampleCutsBuff;
    if (m_cameraBuff) {
...
void ReconstructionCutsGpuRenderThread::EnqueueInitFrameBufferKernel(const bool clearPBO) {
	cl::CommandQueue &oclQueue = m_intersectionDevice->GetOpenCLQueue();
...
void ReconstructionCutsGpuRenderThread::RenderThreadImpl(ReconstructionCutsGpuRenderThread *renderThread)
{
	cerr << "[ReconstructionCutsGpuRenderThread::" << renderThread->m_threadIndex << "] Rendering thread started" << endl;
	...
	const unsigned int pixelCount = renderThread->m_renderEngine->m_film->GetWidth()*
                                    renderThread->m_renderEngine->m_film->GetHeight();
	...
	try {
			...
			for(;;) {
				...
				for (unsigned int i = 0; i < 4; ++i) {
                    ...
                    if (renderThread->m_triLightsBuff) {
						...
						oclQueue.enqueueNDRangeKernel(*(renderThread->m_integratorKernel),
                                                      cl::NullRange,
                                                      cl::NDRange(globalWorkGroupSize),
                                                      cl::NDRange(renderThread->m_initWorkGroupSize),
                                                      NULL, (i == 0) ? &event : NULL);
					}
                    cnt++;
                }
		...
		cerr << "[ReconstructionCutsGpuRenderThread::" << renderThread->m_threadIndex << "] Rendering thread halted" << endl;
	} catch (boost::thread_interrupted) {
		cerr << "[ReconstructionCutsGpuRenderThread::" << renderThread->m_threadIndex << "] Rendering thread halted" << endl;
	} catch (cl::Error err) {
		cerr << "[ReconstructionCutsGpuRenderThread::" << renderThread->m_threadIndex << "] Rendering thread ERROR: " <<
