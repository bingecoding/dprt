...
LightCutsGpuRenderEngine::LightCutsGpuRenderEngine(Scene *scene, Film *film,
...
RenderEngine(scene, film, filmMutex, cfg)
{
    ...
    " OpenCL intersection devices for LightcutsGPU render engine" << endl;
    
    if (m_oclIntersectionDevices.size() < 1)
		throw runtime_error("Unable to find an OpenCL intersection device for\
                            LightcutsGPU render engine");
    
    
	// Create and start render threads
	const size_t renderThreadCount = m_oclIntersectionDevices.size();
	cerr << "Starting "<< renderThreadCount << " LightCutsGPU render threads"
    << endl;
    	
    for (size_t i = 0; i < renderThreadCount; ++i) {
		LightCutsGpuRenderThread *t = new LightCutsGpuRenderThread(
                                                     i,
                                                     m_seed + i * m_globalWorkGroupSize,
...
LightCutsGpuRenderEngine::~LightCutsGpuRenderEngine() {
	if (m_started) {
		Interrupt();
		Stop();
	}
...
void LightCutsGpuRenderEngine::Start() {
	RenderEngine::Start();
...
void LightCutsGpuRenderEngine::Interrupt() {
	for (size_t i = 0; i < m_renderThreads.size(); ++i)
		m_renderThreads[i]->Interrupt();
}

void LightCutsGpuRenderEngine::Stop() {
	RenderEngine::Stop();
...
unsigned int LightCutsGpuRenderEngine::GetThreadCount() const {
	return m_renderThreads.size();
}

void LightCutsGpuRenderEngine::UpdateFilm()
{
...
unsigned int LightCutsGpuRenderEngine::GetPass() const {
