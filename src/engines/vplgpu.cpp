...
VPLGPURenderEngine::VPLGPURenderEngine(Scene *scene, Film *film,
...
RenderEngine(scene, film, filmMutex, cfg)
{
	...
	cerr << "Starting "<< renderThreadCount << " PathGPU render threads"
    << endl;
	
    for (size_t i = 0; i < renderThreadCount; ++i) {
		VPLGPURenderThread *t = new VPLGPURenderThread(
                                                     i,
...
VPLGPURenderEngine::~VPLGPURenderEngine() {
	if (m_started) {
		Interrupt();
		Stop();
	}
...
void VPLGPURenderEngine::Interrupt() {
	for (size_t i = 0; i < m_renderThreads.size(); ++i)
		m_renderThreads[i]->Interrupt();
}

void VPLGPURenderEngine::Stop() {
	RenderEngine::Stop();
...
unsigned int VPLGPURenderEngine::GetThreadCount() const {
	return m_renderThreads.size();
}

void VPLGPURenderEngine::UpdateFilm()
{
...
unsigned int VPLGPURenderEngine::GetPass() const {
