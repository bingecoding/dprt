...
PathGPURenderThread::PathGPURenderThread(const unsigned int index,
...
void PathGPURenderThread::Start() {
    ...
    m_renderThread = new boost::thread(boost::bind(PathGPURenderThread::RenderThreadImpl, this));
    
    // Set renderThread priority
    bool res = SetThreadRRPriority(m_renderThread);
    if (res && !m_reportedPermissionError) {
        cerr << "[PathGPURenderThread::" << m_threadIndex << "] Failed to set ray intersection thread priority (you probably need root/administrator permission to set thread realtime priority)" << endl;
...
void PathGPURenderThread::InitRender()
{
	...
	cerr << "[PathGPURenderThread::" << m_threadIndex <<
	...
	if (m_kernelsParameters != newKernelParameters) {
        ...
        files.push_back("kernels/pathgpu_kernel.cl");
		...
		cerr << "[PathGPURenderThread::" << m_threadIndex << "] Suggested work group size: " << m_initFBWorkGroupSize << endl;
		...
		if (m_triLightsBuff) {
			...
			m_advancePathStep1Kernel = new cl::Kernel(program,
                                                      "AdvancePaths_Step1");
            
            m_advancePathStep1WorkGroupSize = m_intersectionDevice->
            GetForceWorkGroupSize();
            
            cerr << "[PathGPURenderThread::" << m_threadIndex <<
            "] Forced work group size: " <<
            m_advancePathStep1WorkGroupSize << endl;
        ...
        delete m_advancePathStep2Kernel;
		m_advancePathStep2Kernel = new cl::Kernel(program, "AdvancePaths_Step2");
		
        m_advancePathStep2WorkGroupSize = m_intersectionDevice->GetForceWorkGroupSize();
        cerr << "[PathGPURenderThread::" << m_threadIndex <<
        "] Forced work group size: " << m_advancePathStep2WorkGroupSize << endl;
		
		tEnd = WallClockTime();
		cerr  << "[PathGPURenderThread::" << m_threadIndex <<
        "] Kernels compilation time: " << int((tEnd - tStart) * 1000.0)
        << "ms" << endl;
	} else {
		cerr << "[PathGPURenderThread::" << m_threadIndex <<
        "] Using cached kernels" << endl;
    }
    ...
    if (m_triLightsBuff) {
		...
		m_advancePathStep1Kernel->setArg(argIndex++, *m_pathsBuff);
		m_advancePathStep1Kernel->setArg(argIndex++, *m_raysBuff);
		m_advancePathStep1Kernel->setArg(argIndex++, *m_hitsBuff);
		m_advancePathStep1Kernel->setArg(argIndex++, *m_materialsBuff);
		m_advancePathStep1Kernel->setArg(argIndex++, *m_meshMatsBuff);
		m_advancePathStep1Kernel->setArg(argIndex++, *m_meshIDBuff);
		m_advancePathStep1Kernel->setArg(argIndex++, *m_colorsBuff);
		m_advancePathStep1Kernel->setArg(argIndex++, *m_normalsBuff);
		m_advancePathStep1Kernel->setArg(argIndex++, *m_trianglesBuff);
        m_advancePathStep1Kernel->setArg(argIndex++, *m_triLightsBuff);
        // Textures
        if (m_texMapRGBBuff)
            m_advancePathStep1Kernel->setArg(argIndex++, *m_texMapRGBBuff);
        if (m_texMapAlphaBuff)
            m_advancePathStep1Kernel->setArg(argIndex++, *m_texMapAlphaBuff);
        if (m_texMapRGBBuff || m_texMapAlphaBuff) {
            m_advancePathStep1Kernel->setArg(argIndex++, *m_texMapDescBuff);
            m_advancePathStep1Kernel->setArg(argIndex++, *m_meshTexsBuff);
            m_advancePathStep1Kernel->setArg(argIndex++, *m_uvsBuff);
        }
        
    }
	
    unsigned int argIndex = 0;
	m_advancePathStep2Kernel->setArg(argIndex++, *m_pathsBuff);
	m_advancePathStep2Kernel->setArg(argIndex++, *m_raysBuff);
	m_advancePathStep2Kernel->setArg(argIndex++, *m_hitsBuff);
	m_advancePathStep2Kernel->setArg(argIndex++, *m_frameBufferBuff);
	m_advancePathStep2Kernel->setArg(argIndex++, *m_materialsBuff);
	m_advancePathStep2Kernel->setArg(argIndex++, *m_meshMatsBuff);
	m_advancePathStep2Kernel->setArg(argIndex++, *m_meshIDBuff);
	m_advancePathStep2Kernel->setArg(argIndex++, *m_colorsBuff);
	m_advancePathStep2Kernel->setArg(argIndex++, *m_normalsBuff);
	m_advancePathStep2Kernel->setArg(argIndex++, *m_trianglesBuff);
    // Area lights
	if (m_triLightsBuff)
		m_advancePathStep2Kernel->setArg(argIndex++, *m_triLightsBuff);
    // Textures
    if (m_texMapRGBBuff)
        m_advancePathStep2Kernel->setArg(argIndex++, *m_texMapRGBBuff);
    if (m_texMapAlphaBuff)
        m_advancePathStep2Kernel->setArg(argIndex++, *m_texMapAlphaBuff);
    if (m_texMapRGBBuff || m_texMapAlphaBuff) {
        m_advancePathStep2Kernel->setArg(argIndex++, *m_texMapDescBuff);
        m_advancePathStep2Kernel->setArg(argIndex++, *m_meshTexsBuff);
        m_advancePathStep2Kernel->setArg(argIndex++, *m_uvsBuff);
    }
...
void PathGPURenderThread::RenderThreadImpl(PathGPURenderThread *renderThread) {
	cerr << "[PathGPURenderThread::" << renderThread->m_threadIndex << "] Rendering thread started" << endl;
	...
	try {
		...
		while (!boost::this_thread::interruption_requested()) {
			...
			for(unsigned int j = 0;;++j) {
				...
				for (unsigned int i = 0; i < 4; ++i) {
					// Trace rays
					renderThread->m_intersectionDevice->EnqueueTraceRayBuffer(*(renderThread->m_raysBuff),
                                                                              *(renderThread->m_hitsBuff), globalWorkGroupSize);
                    
					// Advance paths
                    
					if (renderThread->m_triLightsBuff) {
						// Only if direct light sampling is enabled
						oclQueue.enqueueNDRangeKernel(*(renderThread->m_advancePathStep1Kernel), cl::NullRange,
                                                      cl::NDRange(globalWorkGroupSize), cl::NDRange(renderThread->m_advancePathStep1WorkGroupSize));
					}
                    
					if (i == 0)
                        oclQueue.enqueueNDRangeKernel(*(renderThread->m_advancePathStep2Kernel), cl::NullRange,
                                                      cl::NDRange(globalWorkGroupSize), cl::NDRange(renderThread->m_advancePathStep2WorkGroupSize), NULL, &event);
					
                    else
						oclQueue.enqueueNDRangeKernel(*(renderThread->m_advancePathStep2Kernel), cl::NullRange,
                                                      cl::NDRange(globalWorkGroupSize), cl::NDRange(renderThread->m_advancePathStep2WorkGroupSize));
		...
		cerr << "[PathGPURenderThread::" << renderThread->m_threadIndex << "] Rendering thread halted" << endl;
	} catch (boost::thread_interrupted) {
		cerr << "[PathGPURenderThread::" << renderThread->m_threadIndex << "] Rendering thread halted" << endl;
	} catch (cl::Error err) {
		cerr << "[PathGPURenderThread::" << renderThread->m_threadIndex << "] Rendering thread ERROR: " << err.what() << "(" << err.err() << ")" << endl;
	}
