...
#define SCENE_DIAG PARAM_SCENE_DIAG
#define BLOCKWH 4
...
#define PATH_STATE_CAMERA_RAY 0
#define PATH_STATE_CUTS_METHOD 1
#define PATH_STATE_DIRECT_LIGHT 2
#define PATH_STATE_ROOT_LIGHT 3
#define PATH_STATE_LIGHT_CUTS 4
#define PATH_STATE_RECONSTRUCTION_CUTS 5
#define PATH_STATE_RECONSTRUCTION_WAIT 6
#define PATH_STATE_RECONSTRUCTION_CUTS_WAIT 7
#define PATH_STATE_RECONSTRUCTION_SAMPLES 8
#define PATH_STATE_WAIT 9
#define PATH_STATE_TERMINATE 10
...
//------------------------------------------------------------------------------
// Ray intersection code just in the event that the reconstruction cut cannot
// be used to compute the estimated radiance, see path state
// PATH_STATE_RECONSTRUCTION_CUTS in the integrator kernel
//------------------------------------------------------------------------------
...
float3 GetNormal(__global Ray *ray, __global RayHit *rayHit, const uint triIndex,
                 __global Vector *normals, __global Triangle *triangles)
{
    ...
    float3 N = Mesh_InterpolateNormal(normals, triangles,
                                      triIndex,
                                      hitPointB1, hitPointB2);
    N *= (dot(rayDir, N) > 0.f) ? -1.f : 1.f;
                        ...
                        HeapNode *heapData)
{
    ...
    switch (triSurfMat->type) {
        ...
        case MAT_BLINNPHONG: {
            ...
            //float maxAlpha = BoundAngle(node, shadingPoint, shadingNormal, wo);
            //if(maxAlpha == 0.f) maxAlpha = 1.f; // Bound set to max
            float maxAlpha = 1.f;
...
float3 BilinearInterpolation(__global Sample *sampleCuts,  Seed *seed, uint goffsNodeId)
{
    
    const int gid = get_global_id(0);
    float3 intensity[SAMPLESIZE];
    
    for(int i=0; i < SAMPLESIZE; i++) {
        int offset = i * PARAM_LIGHT_COUNT;
        uint offs = goffsNodeId + offset;
        intensity[i] = sampleCuts[offs].intensity;
    }
    
    // Get x and y co-ordinates
    float x = gid % BLOCK_WIDTH; // Local to 4x4 block
    float idx = gid % BLOCKSIZE; // Index in block, blocksize is 16; 4x4 pixels
    float y = idx / BLOCKWH;
    
    x += RndFloatValue(seed) - 0.5f;
    y += RndFloatValue(seed) - 0.5f;
    
    float3 p11 = intensity[0] * (3 - x) * (3 - y);
    float3 p12 = intensity[1] * (3 - x) * (y - 0);
    float3 p21 = intensity[2] * (x - 0) * (3 - y);
    float3 p22 = intensity[3] * (x - 0) * (y - 0);
    float weight = 1.f/((3 - 0) * (3 - 0));
    
    float3 interpolatedIntensity = weight * (p11 + p21 + p12 + p22);
...
__kernel void Init(
    ...
    uint heightShift = PARAM_IMAGE_WIDTH * BLOCK_WIDTH;
    uint blockHeight = gid / heightShift;
    uint shiftPixel = blockHeight * heightShift;
    uint lbidx = gid - shiftPixel; // unique index in block of current row, i.e. a row represents multiple blocks
    uint rbidx = lbidx / BLOCKSIZE; // the block number in the current row
    
    uint xPos = gid % BLOCK_WIDTH; // Local to 4x4 block
    uint idx = gid % BLOCKSIZE; // Index in block, blocksize is 16; 4x4 pixels
    uint yPos = idx / BLOCKWH; // Determine row of 4x4 pixel block
    
    uint cIdx = shiftPixel + (xPos + yPos*PARAM_IMAGE_WIDTH + rbidx*BLOCK_WIDTH);
    
    const uint pixelIndex = cIdx;//(PARAM_STARTLINE * PARAM_IMAGE_WIDTH + gid) % (PARAM_IMAGE_WIDTH * PARAM_IMAGE_HEIGHT);
                   ...
                   )
{
    ...
    if (subpixelIndex >= PARAM_SAMPLE_PER_PIXEL) {
        uint offset = PARAM_PATH_COUNT/BLOCKWH;
        uint row = path->pixelIndex / PARAM_IMAGE_WIDTH;
        uint rowPos = path->pixelIndex - row*PARAM_IMAGE_WIDTH + offset;
        if(rowPos < PARAM_IMAGE_WIDTH) {
            newPixelIndex = path->pixelIndex + offset;
        }else {
            newPixelIndex = path->pixelIndex + (BLOCKWH-1)*PARAM_IMAGE_WIDTH + offset;
        }
        newPixelIndex %= (PARAM_IMAGE_WIDTH * PARAM_IMAGE_HEIGHT);
...
void Integrator(__global Path *paths,
                ...
                __global Sample *sampleCuts,
                __global HeapNode *lightCutsHeap,
                __global int *heapIndex,
                __global bool *standardLightCuts,
                __global Pixel *frameBuffer,
                ...
                )
{
    ...
    // Used for reconstruction cuts
    __local Ray localRays[PARAM_WORK_GROUP_SIZE];
    __local RayHit localRayHits[PARAM_WORK_GROUP_SIZE];
    ...
    // Test if all rays in the 4x4 pixel block have hit something,
    // otherwise revert to standard lightcuts.
    const uint lIdx = gid % BLOCKSIZE;
    const uint bIdx = gid / BLOCKSIZE;
    
    __global bool *stdLightCuts = &standardLightCuts[bIdx]; // Shared data for each 4x4 block
    ...
    if(path->state == PATH_STATE_CAMERA_RAY) {
        ...
        *stdLightCuts = FALSE;
        ...
        localRays[ltid] = *ray;
        Intersect(&localRays[ltid], &localRayHits[ltid], verts,
                  triangles, bvhTree);
        
        *rayHit = localRayHits[ltid];
        
        currentTriangleIndex = localRayHits[ltid].index;
        
        path->state = PATH_STATE_CUTS_METHOD;
    ...
    // The last thread in the 4x4 block does nothing, i.e. lidx + 1 == BLOCKSIZE
    if(path->state == PATH_STATE_CUTS_METHOD) {
        ...
        if(*stdLightCuts || (lIdx + 1) == BLOCKSIZE) {
            return;
        }
        ...
        float3 n1 = Mesh_InterpolateNormal(vertNormals, triangles,
                                           currentTriangleIndex,
                                           hitPointB1, hitPointB2);
        n1 *= (dot(rayDir, n1) > 0.f) ? -1.f : 1.f;
        
        __global Ray *rayAdj = &rays[gid+1];
        __global RayHit *rayHitAdj = &rayHits[gid+1];
        uint adjTriangleIndex = rayHitAdj->index;
        if(adjTriangleIndex == 0xffffffffu) {
            *stdLightCuts = TRUE;
            return;
        }
        
        float3 n2 = GetNormal(rayAdj, rayHitAdj, adjTriangleIndex,
                              vertNormals, triangles);
        float angle = acos(dot(n1, n2)) * 180 / M_PI;
        if(angle > 30.f) {
            *stdLightCuts = TRUE;
            return;
        }
        ...
        // Material test
        uint meshIndex = meshIDs[currentTriangleIndex];
        __global Material *hitPointMat = &mats[meshMats[meshIndex]];
        const uint matType = hitPointMat->type;
        meshIndex = meshIDs[adjTriangleIndex];
        hitPointMat = &mats[meshMats[meshIndex]];
        const uint adjMatType = hitPointMat->type;
        if(matType != adjMatType) {
            *stdLightCuts = TRUE;
            return;
        }
        ...
        const float3 apex = rayOrig + rayHit->t * rayDir;
        
        float h = SCENE_DIAG;
        float r = h / tan(60.f * M_PI / 180.f); // radius of base of cone
        uint offset = gid - lIdx;
        #pragma unroll BLOCKSIZE
        for(int i=0; i < BLOCKSIZE; i++) {
            if(lIdx==i) continue;
            
            rayAdj = &rays[offset+i];
            rayHitAdj = &rayHits[offset+i];
            float3 rayOrigAdj = VLOAD3F(&rayAdj->o.x);
            float3 rayDirAdj = VLOAD3F(&rayAdj->d.x);
            const float3 p = rayOrigAdj + rayHitAdj->t * rayDirAdj;
            float dist = dot(p - apex, n1);
            
            if(0 <= dist <= h){
                float radius = (dist / h) * r;
                float3 v = (p - apex) - (dist * n1);
                float orth = length(v);
                if(orth < radius) {
                    // Point inside cone
                    *stdLightCuts = TRUE;
    ...
    if(path->state == PATH_STATE_DIRECT_LIGHT) {
        ...
        if (currentTriangleIndex != 0xffffffffu) {
            ...
            if (directLightPdf > 0.f) {
                ...
                // Add direct radiance
                if (localRayHits[ltid].index == 0xffffffffu) {
            ...
            //if(!(*stdLightCuts)) // This should never be the case because if nothing is hit then stdLightCuts is set to false in PATH_STATE_CUTS_METHOD
            //path->state = PATH_STATE_RECONSTRUCTION_CUTS_WAIT;
            //else
            //path->state = PATH_STATE_TERMINATE;
            path->state = PATH_STATE_WAIT;
        }
    ...
    // Place root on heap.
    // We assume the root node is not visible.
    if(path->state == PATH_STATE_ROOT_LIGHT) {
        ...
        *Idx = 0;
        ...
        HeapNode heapData;
        float shadowRay = GetClusterRadiance(node, hitPointMat, hitPointP,
                                             shadeN, wo, &heapData);
        ...
        if(shadowRay) {
            ...
            Intersect(&localRays[ltid], &localRayHits[ltid], verts,
                      triangles, bvhTree);
            uint hitCluster = localRayHits[ltid].index;
            if(hitCluster != 0xffffffffu) {
                heapData.estimatedRadiance *= 0.f; // Cluster not visible
            }
        }

        
        path->depth++; // used to count shadow rays
        path->lightCount++;
        
        bool sampleCorner = (lIdx == 0) ? TRUE : (lIdx == 3) ? TRUE : (lIdx == 12) ? TRUE : (lIdx == 15) ? TRUE : FALSE;
        if(!(*stdLightCuts) && !sampleCorner) {
            // Non-sample pixels in the 4x4 block wait for the samples to be
            // computed.
            path->state = PATH_STATE_RECONSTRUCTION_CUTS_WAIT;
        } else {
            //path->state = PATH_STATE_CLUSTER_VISIBILITY;
            
            if(sampleCorner) {
                uint loffs = (lIdx == 0) ? 0 : (lIdx == 3) ? 1 : (lIdx==12) ? 2 : 3;
                uint goffs = SAMPLESIZE * PARAM_LIGHT_COUNT * bIdx;
                uint offs = loffs*PARAM_LIGHT_COUNT + goffs;
                
                __global Sample *sampleCut;
                sampleCut = &sampleCuts[offs + node->ID];
                sampleCut->intensity = 0.f;
    ...
    if(path->state == PATH_STATE_LIGHT_CUTS) {
        ...
        float3 totalRadiance = path->accumRadiance;
        int index;
        HeapNode top;
        ...
        for(int i=0; i < 4 && !Empty(dheap, Idx); i++) {
            ...
            if((top.errorBound <= (ERROR_RATIO * Y(totalRadiance))) || ((path->lightCount) >= MAXIMUM_CUT_SIZE) )
            {
                ...
                if(*stdLightCuts) {
                    //path->accumRadiance = (float3)(255.f,0.f,0.f);
                    path->state = PATH_STATE_WAIT;
                    //path->state = PATH_STATE_TERMINATE;
                } else {
                    // Used for reconstruction cuts by remaining 12 threads in 4x4 block
                    //path->accumRadiance = (float3)(255.f,0.f,0.f);
                    
                    path->state = PATH_STATE_RECONSTRUCTION_SAMPLES;
                }
                return;
            }
            ...
            */
            ...
            for(int i=0; i < ARITY; i++) {
                ...
                if(idx == repId) {
                    ...
                    HeapNode heapData;
                    float shadowRay = GetClusterRadiance(sibling, hitPointMat, hitPointP,
                                                         shadeN, wo, &heapData);
        ...
        if(*stdLightCuts) {
            ...
            //path->state = PATH_STATE_RECONSTRUCTION_WAIT;
            path->state = PATH_STATE_RECONSTRUCTION_SAMPLES;
        }
#else
        if(Empty(dheap, Idx)) {
    ...
    if(path->state == PATH_STATE_RECONSTRUCTION_SAMPLES) {
        
        uint lcoffs = (lIdx == 0) ? 0 : (lIdx == 3) ? 1 : (lIdx==12) ? 2 : 3;
        uint goffs = SAMPLESIZE * PARAM_LIGHT_COUNT * bIdx;
        uint offs = lcoffs*PARAM_LIGHT_COUNT + goffs;
        
        __global Sample *lightCut;
        __global Sample *sampleCut;
        int heapIdx = *Idx;
        uint clusterIdx;
        for(int i=0; i < heapIdx; i++) {
            clusterIdx = dheap[i].id;
            lightCut = &sampleCuts[offs+clusterIdx];
            lightCut->intensity = dheap[i].estimatedRadiance;
            
            float parent = floor((convert_float(clusterIdx) - 1.f) / 2.f);
            while(parent >= 0.f) {
                int pid = convert_int(parent);
                sampleCut = &sampleCuts[offs + pid];
                
                sampleCut->intensity += lightCut->intensity;
    ...
    if(path->state == PATH_STATE_RECONSTRUCTION_CUTS) {
        ...
        for(int i=0; i < 4 && !Empty(dheap, Idx); i++) {
            ...
            HeapNode top = dheap[0];
            int index = top.id;
            
            // Remove element and update heap
            Pop(dheap, Idx);
            ...
            float minTotalIntensity = INFINITY;
           
            uint goffs = SAMPLESIZE * PARAM_LIGHT_COUNT * bIdx;
            __global Sample *sampleCut;
            __global Sample *rootSample;
            for(int i=0; i < SAMPLESIZE; i++) {
                int offset = i * PARAM_LIGHT_COUNT;
                uint offs = goffs + offset + node->ID;
                sampleCut = &sampleCuts[offs];
                rootSample = &sampleCuts[goffs + offset];
                
                if(maxIntensity < Y(sampleCut->intensity)) {
                    maxIntensity = Y(sampleCut->intensity);
                }
                if(minIntensity > Y(sampleCut->intensity)) {
                    minIntensity = Y(sampleCut->intensity);
                }
                if(minTotalIntensity > Y(rootSample->intensity)) {
                    minTotalIntensity = Y(rootSample->intensity);
                }
            }

            float threshold = ERROR_RATIO * minTotalIntensity;
            
            if(maxIntensity == 0.f) {
                
                path->lightCount++;
            
            } else if((((maxIntensity - minIntensity) < threshold) && (minIntensity > 0)) ) {
                
                path->lightCount++;
                
                float3 interpolatedIntensity = BilinearInterpolation(sampleCuts,
                                                                     &seed,
                                                                     goffs+node->ID);
                totalRadiance += interpolatedIntensity;
                
            } else if(((maxIntensity - minIntensity) < threshold) || node->isLeaf) {
                ...
                if(shadowRay|| shadowRay == -2.f) {
                    ...
                    if(hitCluster != 0xffffffffu) {
                        heapData.estimatedRadiance *= 0.f; // Cluster not visible, something was hit
                    }
        ...
        if(Empty(dheap, Idx)) {
            ...
            // Update radiance
            path->accumRadiance = totalRadiance;
            path->state = PATH_STATE_RECONSTRUCTION_CUTS;
        }
    ...
    // Threads which need samples to compute their radiance must
    // wait until the samples are ready.
    // When all threads of a 4x4 enter
    if(path->state == PATH_STATE_RECONSTRUCTION_CUTS_WAIT) {
        
        //----------------------------------------------------------------------
        // Some threads do not hit anything in the scene when in
        // PATH_STATE_DIRECT_LIGHT, that is why we need to test the
        // PATH_STATE_WAIT.
        //----------------------------------------------------------------------
        
        // Only the first thread of the 4x4 block checks if all other
        // threads are now in this state.
        // The first thread is also a thread which computes one of the four
        // samples.
        
        int ltidx = gid % BLOCKSIZE;
        if(ltidx == 0) {
            bool changeState = TRUE;
            #pragma unroll BLOCKSIZE
            for(int i=1; i< BLOCKSIZE; ++i) {
                __global Path *blockPath = &paths[gid + i];
                if(blockPath->state != PATH_STATE_RECONSTRUCTION_CUTS_WAIT) {
                    changeState = FALSE;
                }
            }
            
            // Change state for 4x4 block
            if(changeState) {
                #pragma unroll BLOCKSIZE
                for(int i=0; i< BLOCKSIZE; ++i) {
                    __global Path *blockPath = &paths[gid + i];
                    bool sampleCorner = (i == 0) ? TRUE : (i == 3) ? TRUE : (i == 12) ? TRUE : (i == 15) ? TRUE : FALSE;
                    if(sampleCorner) {
                        // Samples have already been computed, they just
                        // wait for the remaing pixels too finish
                        blockPath->state = PATH_STATE_WAIT;
                    } else {
                        blockPath->state = PATH_STATE_RECONSTRUCTION_CUTS;
                    }
                }
            }
        }
    ...
    if(path->state == PATH_STATE_WAIT) {
        
        // Check if all threads in a 4x4 block have reached this state
        // Only first thread does check
        int ltidx = gid % BLOCKSIZE;
        if(ltidx == 0) {
            bool changeState = TRUE;
            for(int i=0; i< BLOCKSIZE; i++) {
                __global Path *blockPath = &paths[gid + i];
                if(blockPath->state != PATH_STATE_WAIT) {
                    changeState = FALSE;
                }
            }
            
            // Advance state for 4x4 block
            if(changeState) {
                
                for(int i=0; i< BLOCKSIZE; i++) {
                    __global Path *blockPath = &paths[gid + i];
                    blockPath->state = PATH_STATE_TERMINATE; // Finish block
                }
    ...
    if(path->state == PATH_STATE_TERMINATE) {
        
        float3 radiance = path->accumRadiance;
        ...
        radiance = GetColor(f, 0.f, 1.f);
        //printf("colormap: %f %f %f\n",radiance.x, radiance.y, radiance.z);
        //printf("cuts: %d shadow rays: %d\n", path->lightCount, path->depth);
#endif
